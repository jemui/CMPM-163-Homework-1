	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
                ...
                // Affects output texture 
                if (C.r >= 0.5) { //cell is alive
                    if (cnt == 2) {
                        //Any live cell with two live neighbours has the pinkish color and lives on to the next generation.
                    
                        return float4(1.0,0.9,0.9,1.0);
                    } else if (cnt == 3) {
                        //Any live cell three live neighbours has a yellow color and lives on to the next generation.
                        ...
                        // Cyan color for dead cells
                        return float4(0.0,0.7,0.8,1.0);
                    }
                } else { //cell is dead
                    if (cnt == 3) {
